...
if(isset($_POST["submit"]))
    {
        ...
        $cat = '1';
        $rows = $_POST["rows"];
        $date = $_POST["Date_Of_Recipt"];
        $VN = $_POST["Vehicle_Number"];
        $IT = $_POST["In_Time"];
        $INV = $_POST["DC_INV_No_and_Date"];
        $Consigner = $_POST["Consigner_Name_and_Address"];
        $GRD = $_POST["Goods_Reciveing_Department"];
        $image = $_POST["image1"];
        $image = "<img src = ".$image.">";
    ...
    if(mysqli_query($conn,$s))
    {
        ...
        {       
                ...
                $sql = "insert into master(catergory,Date_Of_Reciept,Vehicle_Number,In_Time,DC_INV_No_and_Date,Consigner_Name_and_Address,Goods_Reciveing_Department,Reciver_Signature,Security_Signature,Remarks,Token_Number) values('$cat','$date','$VN','$IT','$INV','$Consigner','$GRD','$image','$Security_Signature','$Remarks','$Token');";
                ...
                if(mysqli_query($conn,$sql))
                    {
                        ...
                        $alert[]= $msg;
                        $alert[]= $msg2;
                        logEvent('----Material Inward Form Session Over---');
                    }
                else{
...
<body>
<div class = "main"><hr>
    <?php logEvent('----Material Inward Form Session Started---'); ?>
    <h1><b><i>Material Inward Form</i></b></h1><hr>


     <div class = "error" padding-bottom = "5px"><?php foreach($alert as $a) {echo "--> ".$a."<br>";}?></div>
    <?php logEvent('Form Loading'); ?>

    <form method = "POST" action = "material_inward.php" id="myform" autocomplete="off" >
    <input type = "hidden" name = "rows" id = "rows" value = ""/>
    <table id = "first">
    <tr><td>Date of Recipt:</td><td ><input type="date" name="Date_Of_Recipt" id = "Date" value = "<?php echo $curr_date; ?>" /></td></tr>
   
    <tr><td>Vehicle No.: </td><td> <input type="text" name="Vehicle_Number" required /></td></tr>
    <tr><td>In Time: </td><td> <input type="time" name="In_Time" value = "<?php echo $curr_time; ?>" /></td></tr>
    <tr><td>DC / INV NO and Date: </td><td> <input type="text" name="DC_INV_No_and_Date" required /></td></tr>
    <tr><td>Consignor Name and Address: </td><td> <input type="text" name="Consigner_Name_and_Address" required /></td></tr>
    ...
    <tr><td>Goods Reciveing Department: </td><td> <input type="text" name="Goods_Reciveing_Department"  required /></td></tr>
    <tr>
    ...
    <?php logEvent('Material Inward Form Load Complete.'); ?>

    <!--Material Inward Form Ends-->
